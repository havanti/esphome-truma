...
void IDFUARTComponent::setup() {
  ...
  err = uart_driver_install(this->uart_num_, this->rx_buffer_size_,
                            ...
                            20, const_cast<QueueHandle_t *>(&this->uart_event_queue_), 0);
  if (err != ESP_OK) {
...
void IDFUARTComponent::set_rx_full_threshold(size_t rx_full_threshold) {
  uart_set_rx_full_threshold(this->uart_num_, (int) rx_full_threshold);
}

void IDFUARTComponent::set_rx_timeout(size_t rx_timeout) {
  uart_set_rx_timeout(this->uart_num_, (uint8_t) rx_timeout);
}
